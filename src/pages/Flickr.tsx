...
export const Flickr = ({ showNext }: PageProps) => {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const albums = (await (await fetch(flickrUserAddress)).json()).photosets
        .photoset
      ...
      const photos = (await (await fetch(flickrAlbumAddress)).json()).photoset
        .photo
